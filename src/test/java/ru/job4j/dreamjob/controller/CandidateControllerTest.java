...
class CandidateControllerTest {
    ...
    public void whenGetByIdVacancyThenGetOnePage() {
        var candidate = new Candidate(1, "name", "desc", now(), 1, 2);
        var model = new ConcurrentModel();
    ...
    public void whenGetByIdVacancyThenGetErrorPageWithMessage() {
        ...
        var model = new ConcurrentModel();
        var view = candidateController.getById(model, 123);
    ...
    @Test
    public void whenUpdateCandidateThenRedirectVacanciesPage() throws IOException {
        var candidate = new Candidate(1, "name", "desc", now(), 3, 2);
        var fileDto = new FileDto(testFile.getOriginalFilename(), testFile.getBytes());
        var candidateArgumentCaptor = ArgumentCaptor.forClass(Candidate.class);
        var fileDtoArgumentCaptor = ArgumentCaptor.forClass(FileDto.class);
    ...
    @Test
    public void whenUpdateCandidateThenExceptionThrownThenGetErrorPageWithMessage() {
        var expectedException = new RuntimeException("Failed to write file");
    ...
    @Test
    public void whenDeleteCandidateThenRedirectVacanciesPage() {
        var idArgumentCaptor = ArgumentCaptor.forClass(Integer.class);
        when(candidateService.deleteById(idArgumentCaptor.capture())).thenReturn(true);

        var model = new ConcurrentModel();
        var view = candidateController.delete(model, 12);
        var actualId = idArgumentCaptor.getValue();

...
public class VacancyControllerTest {
    ...
    @Test
    public void whenGetByIdVacancyThenGetOnePage() {
        var vacancy = new Vacancy(1, "test", "desc", now(), true, 1, 1);
        var model = new ConcurrentModel();
    ...
    @Test
    public void whenGetByIdVacancyThenGetErrorPageWithMessage() {
        when(vacancyService.findById(any(Integer.class))).thenReturn(Optional.empty());

        var model = new ConcurrentModel();
        var view = vacancyController.getById(model, 123);
    ...
    @Test
    public void whenUpdateVacancyThenRedirectVacanciesPage() throws IOException {
        var vacancy = new Vacancy(1, "test", "desc", now(), true, 2, 3);
        var fileDto = new FileDto(testFile.getOriginalFilename(), testFile.getBytes());
        var vacancyArgumentCaptor = ArgumentCaptor.forClass(Vacancy.class);
        var fileDtoArgumentCaptor = ArgumentCaptor.forClass(FileDto.class);
    ...
    @Test
    public void whenUpdateVacancyThenGetErrorPageWithMessage() {
    ...
    @Test
    public void whenUpdateVacancyThenExceptionThrownThenGetErrorPageWithMessage() {
        var expectedException = new RuntimeException("Failed to write file");
    ...
    @Test
    public void whenDeleteVacancyThenRedirectVacanciesPage() {
        var idArgumentCaptor = ArgumentCaptor.forClass(Integer.class);
        when(vacancyService.deleteById(idArgumentCaptor.capture())).thenReturn(true);

        var model = new ConcurrentModel();
        var view = vacancyController.delete(model, 12);
        var actualId = idArgumentCaptor.getValue();

        assertThat(view).isEqualTo("redirect:/vacancies");
        assertThat(actualId).isEqualTo(12);
    }
